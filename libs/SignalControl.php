...
trait SignalControl
{
	...
	protected function getPresenter(): Presenter
	{
		...
		if (!$presenter instanceof Presenter) {
			throw new InvalidStateException(sprintf('%s is not attached to a presenter', static::class));
		}
		return $presenter;
	}
	...
	protected function link(string $destination, array $args = []): string
	{
		...
		if (substr_compare($destination, '!', -strlen('!')) !== 0 || strpos($destination, ':') !== false) {
			throw new InvalidArgumentException(sprintf('%s support only own signals.', static::class));
		}

		$fullPath = strncmp($destination, '//', strlen('//')) === 0;
		if ($fullPath) {
